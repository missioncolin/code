...
$hasPermission = false;
if ($auth->has_permission("modifyTwitter")){
    $hasPermission = true;
}
if ($hasPermission){

$meta['title'] = 'Twitter Account Manager';
...
$primaryTableName = "tblTwitter";

//editable fields
$fields[] = array(
	'label'   => "Handle",
	'dbColName'  => "handle",
	'tooltip'   => "The name of the twitter account",
	'writeOnce'  => false,
...
$fields[] = array(
	'label'   => "Active",
	'dbColName'  => "sysStatus",
	'tooltip'   => "Disable this account if you don't want it displayed on the website",
	'writeOnce'  => false,
		...
		foreach ($fields as $dbField) {
			...
			if ($dbField['dbColName'] != false) {
				...
				if ($dbField['dbColName'] == 'sysStatus') {
                    ...
                    $fieldColValue = "'" . ltrim($db->escape($_POST[$requestFieldID]),"@") . "',";
				    $fieldColNames .= "" . $dbField['dbColName'] . " = " . $fieldColValue;
                }
...
<p>This allows the ability to set which twitter accounts will be visible on the website.</p>
...
case "edit": //show an editor for a row (existing or new)
    ...
    if (is_array($sites)){
    	...
    	if (is_numeric($_GET['id']) && $_GET['id'] > 0) { //if an ID is provided, we assume this is an edit and try to fetch that row from the single table
	...
	//print the base fields
        ...
        $formBuffer .= '<tr><td><strong>Site</strong></td><td>'.$domains[$fieldValue["siteID"]].'</td></tr>';
...
default: //(list)
    ...
    if (is_array($sites)){
    	//list table query:
    	$listqry = sprintf("SELECT `itemID`, concat('@',`handle`) AS handle, `sysStatus`, (SELECT `domain` FROM `sysSitesDomains` AS sd WHERE sd.`siteID` = $primaryTableName.`siteID` ORDER BY myOrder LIMIT 1) AS domain FROM $primaryTableName WHERE sysOpen = '1' AND `siteID` IN (%s)",
    	   implode(",",$sites)
    	);
