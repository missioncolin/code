...
if (!empty($_POST) && isset($_POST["sub-sign-up"]) && validate_form($_POST)) {
    ...
    if (isset($lists["total"]) && $lists["total"] == 1){
        ...
        if ($groups[0]["name"] == "Newsletter"){
            ...
            foreach($subs as $intGroup){
                if ($intGroup["name"] == "Doctors Only" && $this->siteID == 2){
    ...
    if ($newsletter != ""){
        if ($mc->listSubscribe($mcLID, $db->escape(trim($_POST["RQvalMAILEmail_Address"]),true), array(
            ...
            $sent = 1;
            $adminEmail = $db->return_specific_item(false, 'sysStorageTable', 'value', false, "`application` = 'doctors-only'");
            if ($adminEmail !== false){
                $message = "The following person has requested access to the Doctor's Only section of CCOK:\n\nName: ".trim($_POST["RQvalALPHFirstName"]." ".trim($_POST["RQvalALPHLastName"])."\n";
                $message .= "Email: ".trim($_POST["RQvalMAILEmail_Address"])."\n\n";
                $message .= "Login to http://".$_SERVER["SERVER_NAME"]."/admin to review and send credentials\n\n";
                $message .= "--------------\nSent From ".$_SERVER["SERVER_NAME"];
                
                mail($adminEmail,"Doctors-Only Registration",$message);
            }
        }
        else{
...
<div class="blank" style="width:460px !important;">
	...
	<form action="<?php print $_SERVER['REQUEST_URI']; ?>" method="post">
		<table>
			...
			<tr>
				<td colspan="2"><div class="submitWrap"><input type="submit" value="Register" name="sub-sign-up" class="btnStyle blue" /></div></td>
			</tr>
		</table>
	</form>
</div>
